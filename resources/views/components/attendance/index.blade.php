<div x-data="{
        currentView: 'classes', // 'classes' or 'class-attendance'
        selectedClass: {
            grade: 'Grade 4 – Rizal',
            subject: 'Science',
            students: 32,
        ...
        selectClass(item) {
            ...
            this.currentView = 'class-attendance';
        }
     }"
     @back-to-classes.window="currentView = 'classes'"
     @toast.window="showToast($event.detail)"
     @toast-with-undo.window="showToast($event.detail.msg, true)"
    ...
    <div x-show="toastVisible" 
         ...
         class="fixed top-5 right-5 z-50 bg-[#172033] dark:bg-slate-800 text-white px-5 py-3 rounded-2xl shadow-xl border border-slate-700 flex items-center gap-4 text-xs font-bold"
         style="display: none;">
        <div class="flex items-center gap-2">
            <span class="text-teal-400">✨</span>
            <span x-text="toastMessage"></span>
        </div>
        <button x-show="hasUndo" @click="toastVisible = false; $dispatch('undo-last')" class="px-2.5 py-1 bg-[#159A9C] text-white rounded-lg hover:underline text-[11px] font-extrabold cursor-pointer">
            Undo
        </button>
    </div>
    ...
    <!-- ============================================================ -->
    <!-- PAGE 1 — ATTENDANCE LANDING VIEW -->
    <!-- ============================================================ -->
    <div x-show="currentView === 'classes'" 
        ...
        <!-- Header Banner with Soft Clipboard/Clock SVG Illustration -->
        <div class="bg-white dark:bg-[#111C38] border border-[#E5ECEB] dark:border-slate-800 rounded-3xl p-6 md:p-8 flex items-center justify-between shadow-2xs relative overflow-hidden">
            <div class="z-10 max-w-xl">
                ...
                <p class="text-xs md:text-sm text-[#64748B] dark:text-slate-400 font-medium mt-2 leading-relaxed">
                    Choose a class to view and take attendance.
                </p>
            </div>

            <!-- Soft Clipboard & Clock SVG Illustration matching image -->
            <div class="flex items-center gap-4 z-10 shrink-0">
                <div class="hidden sm:block w-24 h-20 text-[#159A9C]/40 dark:text-teal-400/40">
                    <svg viewBox="0 0 100 80" fill="none" class="w-full h-full">
                        <!-- Clipboard -->
                        <rect x="20" y="15" width="45" height="55" rx="6" fill="#DDF6EF" class="dark:opacity-30"/>
                        <rect x="32" y="10" width="21" height="8" rx="3" fill="#159A9C"/>
                        <path d="M30 30 L55 30" stroke="#159A9C" stroke-width="2.5" stroke-linecap="round"/>
                        <path d="M30 40 L50 40" stroke="#159A9C" stroke-width="2" stroke-linecap="round"/>
                        <path d="M30 50 L42 50" stroke="#159A9C" stroke-width="2" stroke-linecap="round"/>
                        <!-- Clock -->
                        <circle cx="70" cy="50" r="18" fill="#FFF4D6" stroke="#D97706" stroke-width="2"/>
                        <path d="M70 40 L70 50 L78 50" stroke="#D97706" stroke-width="2.5" stroke-linecap="round"/>
                    </svg>
                </div>

                <button @click="activeTab = 'classes'; setTimeout(() => $dispatch('open-create-class'), 100)" 
        ...
        <div>
            <div class="flex items-center justify-between mb-5">
                ...
                <span class="text-xs font-semibold text-[#64748B] dark:text-slate-400">Select a class to take daily attendance</span>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                
                <!-- Class 1: Grade 4 - Rizal -->
                <x-attendance.class-card @click="selectClass(classes[0])"
                                         grade="Grade 4 – Rizal"
                                         subject="Science"
                                         :studentsCount="32"
                                         iconBg="bg-[#DDF6EF] dark:bg-[#159A9C]/20"
                ...
                </x-attendance.class-card>

                <!-- Class 2: Grade 5 - Bonifacio -->
                <x-attendance.class-card @click="selectClass(classes[1])"
                                         grade="Grade 5 – Bonifacio"
                                         subject="Mathematics"
                                         :studentsCount="28"
                                         iconBg="bg-[#EEE9FF] dark:bg-purple-950/30"
                ...
                <x-attendance.class-card @click="selectClass(classes[2])"
                                         grade="Grade 3 – Mabini"
                                         subject="English"
                                         :studentsCount="26"
                                         iconBg="bg-[#FFF4D6] dark:bg-amber-950/30"
                ...
                <x-attendance.class-card @click="selectClass(classes[3])"
                                         grade="Grade 6 – Luna"
                                         subject="Araling Panlipunan"
                                         :studentsCount="30"
                                         iconBg="bg-[#FDE9E7] dark:bg-rose-950/30"
                ...
                <x-attendance.class-card @click="selectClass(classes[4])"
                                         grade="Grade 4 – Aguinaldo"
                                         subject="English"
                                         :studentsCount="29"
                                         iconBg="bg-[#EAF2FF] dark:bg-blue-950/30"
                        ...
                        <div class="flex items-center gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-[#64748B] dark:text-slate-400 font-medium mt-1 leading-snug">
                                    Add a new class and start managing attendance.
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    <!-- ============================================================ -->
    <!-- PAGE 2 — CLASS ATTENDANCE VIEW -->
    <!-- ============================================================ -->
    <div x-show="currentView === 'class-attendance'"
         x-transition:enter="transition ease-out duration-200"
         x-transition:enter-start="opacity-0 translate-y-1"
         x-transition:enter-end="opacity-100 translate-y-0"
         style="display: none;">
        
        <x-attendance.class-attendance />

    </div>

    <!-- Modals & Drawers -->
    <x-attendance.take-attendance-modal />
    <x-attendance.generate-sf2-modal />
